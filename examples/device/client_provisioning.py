...
def main():
    ...
    We can provide the following parameters to provisioning function:
      ...
      port=1883 - not required - MQTT port of Ticos instance
      device_name=None - may be generated on Ticos - You may pass here name for device, if this parameter is not assigned, the name will be generated

      ### Credentials type = ACCESS_TOKEN

      access_token=None - may be generated on Ticos - You may pass here some access token and it will be saved as accessToken for device on Ticos.
    ...
    # Call device provisioning, to do this we don't need an instance of the TicosDeviceMqttClient to provision device

    credentials = TicosDeviceMqttClient.provision("127.0.0.1", "PROVISION_DEVICE_KEY", "PROVISION_DEVICE_SECRET")

    if credentials is not None:
        client = TicosDeviceMqttClient("127.0.0.1", 1883, credentials)
        client.connect()
