...
class TicosHTTPAPIException(Exception):
    """Ticos HTTP Device API Exception class."""


class TicosProvisionFailure(TicosHTTPAPIException):
    """Exception raised if device provisioning failed."""


class TicosHTTPDevice:
    """Ticos HTTP Device API class.

    :param host: The Ticos hostname.
    ...
    def __repr__(self):
        return f'<Ticos ({self.host}) HTTP device {self.name}>'

    @property
    def host(self) -> str:
        """Get the Ticos hostname."""
    ...
    def logger(self) -> logging.Logger:
        ...
        return logging.getLogger('TicosHTTPDevice')
    ...
    def connect(self) -> bool:
        """Publish an empty telemetry data to Ticos to test the connection.

        :return: True if connected, false otherwise.
        """
        if self.test_connection():
            self.logger.info('Connected to Ticos')
            self.start_publish_worker()
            return True
        return False

    def _publish_data(self, data: dict, endpoint: str, timeout: int = None) -> dict:
        """Send POST data to Ticos.
    ...
    def _get_data(self, params: dict, endpoint: str, timeout: int = None) -> dict:
        """Retrieve data with GET from Ticos.

        :param params: A dictionary with the parameters for the request.
        :param endpoint: The receiving API endpoint.
        :param timeout: Override the instance timeout for this request.
        :return: A dictionary with the response from the Ticos instance.
        """
        response = self.__session.get(
            url=f'{self.api_base_url}/{endpoint}',
            params=params,
    ...
    def send_telemetry(self, telemetry: dict, timestamp: datetime = None, queued: bool = True):
        """Publish telemetry to Ticos.

        :param telemetry: A dictionary with the telemetry data to send.
        :param timestamp: Timestamp to set for the values. If not set the Ticos server uses
    ...
    def send_rpc(self, name: str, params: dict = None, rpc_id: int = None) -> dict:
        """Send RPC to Ticos and return response.
    ...
    def provision(cls, host: str, device_name: str, device_key: str, device_secret: str):
        ...
        :param host: The root URL to the Ticos instance.
        :param device_name: Name of the device to provision.
        :param device_key: Provisioning device key from Ticos.
        :param device_secret: Provisioning secret from Ticos.
        :return: Instance of :class:`TicosHTTPClient`
        """
        data = {
        ...
        raise TicosProvisionFailure(device)


class TicosHTTPClient(TicosHTTPDevice):
    """Legacy class name."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.logger.critical('TicosHTTPClient class is deprecated, please use TicosHTTPDevice')
